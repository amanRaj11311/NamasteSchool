...
export default function LeaveBalancesScreen() {
  ...
  const [formData, setFormData] = useState({ schoolId: '', staffId: '', leaveTypeId: '', year: new Date().getFullYear().toString(), totalDays: '0' });
  ...
  const handleSave = async () => {
    if (!formData.schoolId || !formData.staffId || !formData.leaveTypeId || !formData.year || !formData.totalDays) { 
      Alert.alert("Error", "Please fill all required fields including School Branch."); return; 
    }
    try {
    ...
    } catch (e: any) { Alert.alert("Error", e.response?.data?.message || "Failed to set balance."); }
  };
        ...
        {isOpen && (
          <View style={styles.dropdownListContainer}>
            <ScrollView nestedScrollEnabled style={{maxHeight: 150}} showsVerticalScrollIndicator={false}>
              {options.map((opt, i) => (
                <TouchableOpacity key={opt.value} style={[styles.dropdownItem, i !== options.length - 1 && styles.borderBottom]} onPress={() => { setFormData({ ...formData, [fieldKey]: opt.value }); setActiveDropdown(null); }}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Set yearly leave quotas per staff member.</Text>
      </View>

      <View style={styles.kpiGrid}>
        <View style={styles.kpiCard}>
          <View style={styles.kpiRow}><View style={[styles.iconCircle, {backgroundColor: '#F3E8FF'}]}><Feather name="bar-chart-2" size={16} color="#D946EF"/></View><Text style={styles.kpiValue}>{balances.length}</Text></View>
          <Text style={styles.kpiLabel}>TOTAL ALLOCATIONS</Text>
        </View>
        <View style={styles.kpiCard}>
          <View style={styles.kpiRow}><View style={[styles.iconCircle, {backgroundColor: '#FEF3C7'}]}><Feather name="users" size={16} color="#F59E0B"/></View><Text style={styles.kpiValue}>{staffList.length}</Text></View>
      ...
      <View style={styles.actionBar}>
        {hasPermission('create') && (
          <TouchableOpacity style={styles.addBtn} onPress={() => {setFormData({ schoolId: '', staffId: '', leaveTypeId: '', year: new Date().getFullYear().toString(), totalDays: '0' }); setFormVisible(true);}}>
            <Feather name="plus" size={14} color="#fff" /><Text style={styles.addBtnText}>Set Balance</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      {loading ? <View style={styles.center}><ActivityIndicator size="large" color="#ef4444" /></View> : (
        <FlatList
          ...
          ListEmptyComponent={<View style={styles.emptyState}><Feather name="pie-chart" size={40} color="#D1D5DB" /><Text style={{color: '#6B7280', marginTop: 10, fontWeight: '500'}}>No balances configured.</Text></View>}
          renderItem={({ item }) => {
              ...
              <View style={styles.card}>
                <View style={styles.cardTop}>
                  <View style={{flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1}}>
                     <View style={styles.avatar}><Text style={styles.avatarText}>{item.staff?.name?.charAt(0) || 'U'}</Text></View>
                     <View style={{ flex: 1 }}>
                       <Text style={styles.staffName} numberOfLines={1}>{item.staff?.name}</Text>
                       <Text style={styles.schoolSubText} numberOfLines={1}>{item.schoolId?.name || 'No Branch'}</Text>
                     </View>
                  </View>
                ...
                <View style={styles.statsGrid}>
                  <View style={styles.statBox}><Text style={styles.statLbl}>TOTAL</Text><Text style={styles.statVal}>{item.totalDays}</Text></View>
                  <View style={styles.statBox}><Text style={styles.statLbl}>CARRY FWD</Text><Text style={[styles.statVal, {color: '#10B981'}]}>{item.carriedForwardDays}</Text></View>
                  <View style={styles.statBox}><Text style={styles.statLbl}>USED</Text><Text style={[styles.statVal, {color: '#ef4444'}]}>{item.usedDays}</Text></View>
                  <View style={styles.statBox}><Text style={styles.statLbl}>REMAINING</Text><Text style={[styles.statVal, {color: '#0ea5e9'}]}>{remaining}</Text></View>
                </View>
              </View>
            );
          }}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          <View style={styles.compactModalContainer}>
            ...
            <ScrollView contentContainerStyle={styles.formScroll} showsVerticalScrollIndicator={false}>
              ...
              {renderInlineDropdown('staffId', 'Staff Member *', staffList.map(s => ({label: s.name, value: s._id})))}
              {renderInlineDropdown('leaveTypeId', 'Leave Type *', leaveTypes.map(t => ({label: t.name, value: t._id})))}

              <View style={styles.row}>
                <View style={{flex: 1, marginRight: 10}}><Text style={styles.inputLabel}>Year *</Text><TextInput style={styles.input} keyboardType="numeric" value={formData.year} onChangeText={t => setFormData({...formData, year: t})} /></View>
                <View style={{flex: 1}}><Text style={styles.inputLabel}>Total Days *</Text><TextInput style={styles.input} keyboardType="numeric" value={formData.totalDays} onChangeText={t => setFormData({...formData, totalDays: t})} /></View>
              </View>

              <TouchableOpacity style={styles.saveBtnFull} onPress={handleSave}><Text style={styles.saveBtnFullText}>Allocate Balance</Text></TouchableOpacity>
            </ScrollView>
          </View>
