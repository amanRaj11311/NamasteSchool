...
export default function NoticeBoardScreen() {
  ...
  const handleSave = async () => {
    if (saving) return; 
    if (!formData.title.trim()) { showToast("Notice title is required.", "error"); return; }
    if (!formData.noticeDate?.trim()) { showToast("Publish date is required.", "error"); return; }

    const targetSchoolId = resolveSchoolId();
    if (!targetSchoolId) {
      if (canSelectSchool) showToast("Please select a school before saving the notice.", "error");
      else showToast("School information is missing. Please login again.", "error");
      return;
    }
  ...
  const renderInlineDropdown = (fieldKey: 'category' | 'status' | 'sendTo', label: string, options: string[], isFilter = false) => {
    ...
    return (
      <View style={[
        ...
        { 
          zIndex: isOpen ? 99 : 1, 
          elevation: isOpen ? 99 : 0,
          flex: isFilter ? 1 : undefined, 
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>Publish announcements for Staff/Students/Guardians.</Text>
        </View>
        
        {hasPermission('create') && (
          <TouchableOpacity style={styles.addBtnTop} onPress={() => openForm()} activeOpacity={0.9}>
            <Feather name="plus" size={16} color="#fff" />
            <Text style={styles.addBtnTextTop}>Create</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.filterSection}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, zIndex: activeFilterDropdown ? 99 : 10, elevation: activeFilterDropdown ? 99 : 10 }}>
          ...
          <View style={styles.detailModalContainer}>
            <View style={styles.detailHeader}>
              ...
              <TouchableOpacity onPress={() => setViewingNotice(null)} style={styles.closeBtnIcon} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                {/* Replaced Icon with native text X */}
                <Text style={{ color: '#4B5563', fontSize: 24, fontWeight: 'bold' }}>✕</Text>
              </TouchableOpacity>
            </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          <View style={styles.compactModalContainer}>
            ...
            <View style={styles.formHeader}>
              ...
              >
                {/* Replaced Vector Icon with foolproof Text symbol to bypass local font mapping glitch */}
            ...
            <ScrollView contentContainerStyle={styles.formScroll} showsVerticalScrollIndicator={false}>
              ...
              {canSelectSchool && (
                <View style={[styles.inputWrapper, { zIndex: activeBranchDropdown ? 99 : 1, elevation: activeBranchDropdown ? 99 : 0 }]}>
              ...
              <View style={[styles.row, { 
                zIndex: (activeDropdown === 'category' || activeDropdown === 'status') ? 99 : 2,
                elevation: (activeDropdown === 'category' || activeDropdown === 'status') ? 99 : 2
              }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                ...
                <TouchableOpacity
                  style={[styles.saveBtnFull, { flex: 1, marginTop: 0 }, saving && { opacity: 0.7 }]}
                  onPress={handleSave}
                  disabled={saving}
                  activeOpacity={0.9}
                >
                  {saving
                    ...
                    : <Text style={styles.saveBtnFullText}>Submit</Text>
                  }
                </TouchableOpacity>
              </View>
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, paddingTop: 16, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#EEF0F2' },
  title: { fontSize: 24, fontWeight: '800', color: '#101317', letterSpacing: -0.3 },
  subtitle: { fontSize: 13, color: '#767C87', marginTop: 4 },
  
  addBtnTop: { backgroundColor: '#B3122A', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 10, shadowColor: '#B3122A', shadowOpacity: 0.25, shadowRadius: 6, shadowOffset: { width: 0, height: 3 }, elevation: 3 },
  addBtnTextTop: { color: '#fff', fontWeight: '800', marginLeft: 6, fontSize: 13 },
  ...
  formHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, backgroundColor: '#B3122A' },
  ...
  formScroll: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 20 },
  ...
  dropdownListContainer: { position: 'absolute', top: 74, left: 0, right: 0, backgroundColor: '#fff', borderWidth: 1, borderColor: '#EEF0F2', borderRadius: 12, elevation: 8, zIndex: 100, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 14 },
  ...
  cancelBtn: { flex: 1, height: 52, borderRadius: 14, backgroundColor: '#F1F2F4', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#E2E4E8' },
  cancelBtnText: { color: '#565C66', fontSize: 15, fontWeight: '800' },
